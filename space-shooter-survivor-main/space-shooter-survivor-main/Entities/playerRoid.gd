...
func _process(delta):
	...
	if abs(get_linear_velocity().x) > maxSpeed or abs(get_linear_velocity().y) > maxSpeed:
		var newSpeed = get_linear_velocity().normalized()
		newSpeed *= maxSpeed
...
func forward(delta):
	if Input.is_action_pressed("boost") and boost_timer > 0.0:
		...
		velocity.x = sin(rotation) * MAX_SPEED * delta * BOOST
		velocity.y = -cos(rotation) * MAX_SPEED * delta * BOOST
	else:
		animationPlayer.play("fly")
		velocity.x = sin(rotation) * MAX_SPEED * delta
		velocity.y = -cos(rotation) * MAX_SPEED * delta
